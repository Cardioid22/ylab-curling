...
- (実行中のサーバージョブなし。全マシン空き。noisy-tree再戦は完了 → 4.3b)
- **[次候補・優先順]**
  ① **発表図表・原稿の更新** (GPW締切接近): fig2の6アーム化は完了 (7ed1d0d)。残り: 深さ対決の
    まとめ図 (3方式の勝率+Δ曲線)、エリアヒートマップ整形、原稿本文。
    ストーリー: 「候補削減は価値で」(A7/A8) + 「クラスタは価値選択と組めば無コストで説明可能」(A9)
    + 「深さは3方式すべてで価値なし=浅い探索+価値選別で十分」(4.3b) で完結。
  ② (任意) d1-noisy vs d1-det 対決 — noisy評価自体の価値の単離 (深さ対決では未測定)
  ③ A9 Phase 2: 分布クラスタリング — ただし動機の一部 (無外乱計画の脆さ) はnoisy再戦で弱まった。
    残る動機は表現・説明可能性側 (外乱込み行動表現の完全実装)
