...
- (実行中のサーバージョブなし。予算スイープ P=100/800 を 2026-09-21 回収 = **GPW 向け計画実験は全て回収済み**)
- **【予算スイープ完結 (2026-09-21)】d(P) 4点曲線** (A9P5N−A1N, 先頭200局面, ex-r1 / 全局面):
  P100 **−0.094**/−0.075 → P200 −0.060/−0.005 → P400 −0.016/+0.044 → P800 −0.002/+0.063。
  低予算優位は単調減少で P800 で消滅、ex-r1 ではどの予算でも負けない (P800 同着 0.242 vs 0.244)。
  全局面平均の反転は r=1 needle のみ (r≤1 バイパスで除去可能)。`scripts/sweep_dP.py`, WAVE1_SUMMARY 第5弾。
- (旧記録: A12N 完了・回収済み 2026-09-03。全 11 アーム×500局面: `gpw_experiment/regret500/stats_500full11`)
- **【A12/A12N 決着 (2026-09-03)】ClusterTS は効かず**: A12N **0.326** (序列 A9P5N 0.309 < A12N 0.326 < A1N 0.334)。
  A12N vs A9P5N raw p=0.015 (A9P5N 優勢 260勝210敗, d=0.12, Holm n.s.)、A12N vs A1N raw p=0.013 (285勝209敗, d=0.03, Holm n.s.)。
  sims 79.9k (A1N 比 +9.4%)。決定的 A12 0.446 も A9P5 0.423 に劣る。位相別: r=1 で A12N 0.484 (A9P5N と同じ退化)、r2-4 0.305 (A9P5N 0.287)。
  = 縮約+Thompson で全クラスタを子に持つと A11 (PW) と同じ幅/深さトレードオフに落ちる。**A12 系は本論文の主表から外す (否定的結果として一行)**。
- **【GPW 査読結果 (2026-09-03 受領, 採否通知 9/4)】** R1: 0 (borderline, Poster) / R2: +1 (weak accept, Oral Short)。指摘:
  R1 ①2章(1)-(4)で既存手法(A2)との差分を明示 ②ケーススタディに正解・既存手法との比較がなく「まとめ」の性能検証と対応しない
  ③正解/既存手法との比較による性能評価を追加 ④既存が失敗し提案が成功する例・その逆の具体例。
  R2 ⑤クラスタリングによる候補削減の「効果」まで示せ ⑥先行研究との関連: VOOT (Kim+ AAAI2020), 状態条件付き行動抽象化 (Kwak+ **UAI2024**; 査読者は ICML と誤記)。
- **【最終原稿 (10/23) に向けた追加実験の優先順位】**
  1. **外乱込み世界のアブレーション (最優先, 4台 2-3日)**: A2N (既存手法+noisy) / A8N (ScoreTopK=クラスタ無し価値上位K+noisy) / A5N (乱択K+noisy) を 500局面×5seed。
     A9P5N の利得を「削減そのもの」「価値選択」「クラスタ構造」に分解 (③⑤)。**A8N≈A9P5N ならクラスタ固有の主張は不可**。`run_reinvest.sh` にアーム行を追加要 (C++ 変更なし)。
  2. **予算スイープ (⑤, +3.6% sims 反論封じ)**: A1N vs A9P5N (+A8N) を P∈{100,400} (K_cap=2/8) で 200局面。regret-予算曲線で「低予算ほどクラスタ有利」を検証。
     P=50 は K_cap=1 で縮退するため `--v-target 25` 等で K≥2 を保つ設計が要る。
  3. **r≤1 スクリーンバイパス (④の限界を修正)**: 実装 + r=1 の 32局面×5seed 再走。A9P5N 平均 −0.02 見込み。
  4. **具体例図 (④)**: 500局面 joined から「medoid 失敗/価値成功」「クラスタが勝つ混戦終盤」「クラスタが害 (非 r1)」を seed 一貫で選び、
     盤面+エリア価値マップ+候補 q のクラスタ色分け (`plot_area_variants.py`)。候補: g7871 e7 s15 / g5335 e1 s14 / g2099 e0 s14, g1616 e0 s14 (A9P5N vs A1N で再確認)。
  5. 任意: A9P5N vs A1N 2エンド自己対戦 (性能の最終証拠だが d=0.09 なので n≥100 ゲーム) / K 感度 (`--v-target 25/100`, 松崎先生の質問)。
- **執筆タスク (計算不要)**: (a) 2章を A2/A9 差分表に (代表選出 medoid→E[score]最大、クラスタ採用 →価値上位K+最良候補所属クラスタ強制、R_pre ロールアウト追加 = sims +8%)
  (b) 関連研究: VOOT (連続行動・決定的環境・Voronoi 分割で楽観サンプリング; 本研究は確率的環境+離散グリッド+結果盤面クラスタ) /
  Kwak+ UAI'24 (潜在ダイナミクスを学習して状態条件付きに部分行動を捨てる; 本研究はシミュレータ結果盤面で on-the-fly に行う無学習版・「効果の等価性」で束ねる) /
  KR-UCT (Yee+ IJCAI2016, カーリング+実行ノイズの連続行動 MCTS) / progressive widening (A11 = 対応する比較, 効かず) / OGA-UCT・ASAP 等の状態行動抽象化
  (c) 代表手品質の表 (best_is_rep, screen_loss, md_screen_loss: A2 vs A9P5) で「より適切に代表手を選べているか」に直接答える
  (d) r=1 退化と時間オーバーヘッド (A9P5N 4715s vs A1N 3926s/局面, ただし走行マシン差あり) を明記。
