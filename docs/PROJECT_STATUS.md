...
- (実行中のサーバージョブなし。全マシン空き。深さ系の実験は4.3b/4.3b-2で完全終了)
- **[次候補・優先順]**
  ① **発表図表・原稿の更新** (最優先): fig2の6アーム化は完了 (7ed1d0d)。残り: 深さまとめ図
    (5経路の証拠: 3対決の勝率+Δ曲線+A1d1regret)、エリアヒートマップ整形、原稿本文。
    ストーリー: 「候補削減は価値で」(A7/A8) + 「クラスタは価値選択と組めば無コストで説明可能」(A9)
    + 「深さは5経路すべてで価値なし=浅い探索+価値選別で十分。学習方策/評価との組合せは今後の課題」で完結。
  ② (任意) d1-noisy vs d1-det 対決 — noisy評価自体の価値の単離 (未測定のまま)
  ③ A9 Phase 2: 分布クラスタリング — 動機は表現・説明可能性側 (外乱込み行動表現の完全実装)
  ④ 中長期: ロールアウト方策・評価関数の学習化 (gPolicy/gEstimator移植) — 深さが復活しうる唯一のレバー
