    ...
    try {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (std::string(argv[i]) == "--ts-obs-var" && i + 1 < argc) {
                ts_obs_var_arg = std::atof(argv[i + 1]);
                if (ts_obs_var_arg <= 0) ts_obs_var_arg = 2.0;
                i++;
            }
            if (std::string(argv[i]) == "--ts-prior-scale" && i + 1 < argc) {
                ts_prior_scale_arg = std::atof(argv[i + 1]);
                if (ts_prior_scale_arg <= 0) ts_prior_scale_arg = 2.0;
                i++;
            }
            ...
            if (std::string(argv[i]) == "--test-id" && i + 1 < argc) {
